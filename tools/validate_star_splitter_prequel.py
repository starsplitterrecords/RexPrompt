#!/usr/bin/env python3
"""Mechanical integrity checks for the Star Splitter Prequel page-production package.
...
DATA = ROOT / "data"
SHOW_DIR = DATA / "shows" / "star-splitter-prequel"
SHOWS = DATA / "shows.json"
SOURCE = DATA / "scenes_prequel.json"
...
REFERENCE_README = ROOT / "production" / "references" / "star-splitter-prequel" / "README.md"
RECOVERY_NOTE = ROOT / "production" / "references" / "star-splitter-prequel" / "recovery-binary-note.md"
...
def main():
    ...
    entries = [s for s in shows if s.get("seriesId") == "star-splitter-prequel"]
    assert len(entries) == 6, f"Expected 6 prequel issue entries, found {len(entries)}"

    all_pages = []
    all_page_ids = set()
    for issue, entry in enumerate(entries, start=1):
        expected_id = f"prequel-e{issue:02d}"
        assert entry.get("id") == expected_id, f"Issue {issue}: show id mismatch"
        assert entry.get("name") == f"Star Splitter Prequel — Issue {issue}", f"Issue {issue}: display name mismatch"
        ...
        expected_file = f"shows/star-splitter-prequel/pages_i{issue:02d}.json"
        ...
        assert entry.get("includeIdPattern") == f"^PREQ_I{issue:02d}_", f"Issue {issue}: include pattern mismatch"
        assert entry.get("scenesFile") != "scenes_prequel.json", f"Issue {issue}: legacy scene file still active"

        pages = load(DATA / expected_file)
        assert len(pages) == 22, f"Issue {issue}: expected 22 pages, found {len(pages)}"
        expected_ids = [f"PREQ_I{issue:02d}_P{page:02d}" for page in range(1, 23)]
        ...
        for index, page in enumerate(pages):
            ...
            assert "episode" not in page and "act" not in page, f"{pid}: legacy scene fields survived"
    ...
    assert Counter(active_dialogue) == Counter(source_dialogue), "Active pages must preserve every legacy source dialogue line exactly once"
    ...
    assert "legacy source" in recovery_text.lower(), "Recovery note still presents old scene IDs as current recipes"
    assert "exact active page recipe" in recovery_text, "Recovery note lacks page-level remapping requirement"

    print("Star Splitter Prequel validation passed")
