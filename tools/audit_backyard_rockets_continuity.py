...
def ep(s):
    ...
    m=re.search(r"S1E\d{2}",s.get("id","") or "")
...
for sid,c in Counter(s.get("id") for s in scenes).items():
    if sid and c>1:
        issues.append({"scene":sid,"episode":"STRUCTURE","kind":"duplicate_scene_id","detail":f"appears {c} times","file":"manifest/overlays"})

counts=Counter(ep(s) for s in scenes)
report={"authority":["released StarSplitterVisions canon","current RexPrompt scene summaries and dialogue","persistent character/faction/setting/region references"],"sceneCount":len(scenes),"episodes":dict(sorted(counts.items())),"totalDialogueLines":sum(len(s.get("dialogueInline",[]) or []) for s in scenes),"qualityGatePassed":not issues,"issueCount":len(issues),"issueKinds":dict(sorted(Counter(i["kind"] for i in issues).items())),"issues":issues,"scenes":rows}
REPORT.write_text(json.dumps(report,indent=2,ensure_ascii=False),encoding="utf-8")
print("BACKYARD ROCKETS CONTINUITY QUALITY GATE")
print("Scenes:",report["sceneCount"],"Episodes:",report["episodes"])
print("Dialogue lines:",report["totalDialogueLines"])
