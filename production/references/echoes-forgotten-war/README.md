...
## Recovered durable identity references

The current recovered visual-reference pack is `visual-reference-pack.json`. Its character images are stored under `characters/` and include:
...
- promotional lineups or labeled character graphics as page-style authority
...
No historical Issue #1 comic-page generation from the recovered chat substantially passed current recipe fidelity and continuity review, so no Echoes recipe-level approved production draft was created from those pages.

The production frontier therefore remains `EFW_S1E01_S01`.
