...
| **Division Threshold** | Coming-soon catalog entry exists in StarSplitterVisions, but there is no released cover, story page, release record, or collected PDF. | Issues 1–8 are compiled at 26 pages each and are registered through the supported `scenesFiles` path. Issue 1 has completed mechanical/assembler review; invalid direct-continuity links on P16, P20 and P26 were repaired. | None. | One creative readiness conflict remains: current Notion authority says Season One opens with Ostra-9 at Checkpoint 9, while the compiled Issue 1 still opens with a four-page historical prologue. Resolve the prologue's disposition before sequential image production. The Visions slug now exists, so the prior source-mapping mismatch is resolved; curated RexPrompt references remain visual authority until actual pages are released. |
...
5. **Division Threshold now has a real Visions catalog entry, but no released visual canon.** The prior source-mapping mismatch is resolved. Issue 1's remaining non-image blocker is the conflict between the four-page historical prologue and the newer Notion instruction that Season One opens at Checkpoint 9.
...
2. **Division Threshold — resolve the Issue 1 opening-structure conflict; all identified mechanical/assembler defects are otherwise repaired.**
